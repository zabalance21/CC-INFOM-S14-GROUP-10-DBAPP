...
public class ServiceDAO {
    ...
    public void addService(Service s) {
        String sql = "INSERT INTO Service(serviceId, name, description, rate, category) VALUES (?,?,?,?,?)";
    ...
    public void updateService(Service s) {
        String sql = "UPDATE Service SET name=?, description=?, rate=?, category=?, availability=? WHERE serviceId=?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            ps.setString(6, s.getServiceId());
            ps.executeUpdate();
            System.out.println("Service updated: " + s.getServiceId());
        } catch (SQLException e) { e.printStackTrace(); }
    }

    // DELETE (placeholder "availability" will change)
    public void deleteService(String serviceId) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            if (rows > 0) System.out.println("Service " + serviceId + " marked as UNAVAILABLE.");
            else System.out.println("No service found with ID: " + serviceId);
        } catch (SQLException e) { e.printStackTrace(); }
    }

    public void markServiceAvailable(String serviceId)     { updateAvailability(serviceId, ServiceAvailability.AVAILABLE); }
    public void markServiceUnavailable(String serviceId)   { updateAvailability(serviceId, ServiceAvailability.UNAVAILABLE); }
    public void markServiceDiscontinued(String serviceId)  { updateAvailability(serviceId, ServiceAvailability.DISCONTINUED); }

    private void updateAvailability(String serviceId, ServiceAvailability a) {
        String sql = "UPDATE Service SET availability = ? WHERE serviceId = ?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, a.db());
            stmt.setString(2, serviceId);
            int rows = stmt.executeUpdate();
            if (rows > 0) System.out.println("Service " + serviceId + " set to " + a.db() + ".");
            else System.out.println("No service found with ID: " + serviceId);
        } catch (SQLException e) { e.printStackTrace(); }
    }
