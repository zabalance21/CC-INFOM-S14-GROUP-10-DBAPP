...
public class ClientDAO {  // FOR SQL CLIENT TABLE QUERIES
    ...
    public List<ClientHistory> getClientHistory(String clientId) {
        ...
        String invoiceSql = "SELECT invoiceId, status, amount, YEAR(invoiceDate) AS year FROM Invoice WHERE clientId = ?";
        String paymentSql = "SELECT paymentId, payment_status, amount, YEAR(paymentDate) AS year FROM Payment WHERE clientId = ?";
