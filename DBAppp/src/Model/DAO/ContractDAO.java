...
public class ContractDAO {
    ...
    public Contract getContractByID(String contractID) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                ContractStatus.valueOf(rs.getString("contractStatus").toUpperCase());
            }
    ...
    public void updateClosedContracts() {
        String sql = "UPDATE Contract SET contract_status = 'CLOSED' WHERE endDate < CURDATE() ";

        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            int rowsUpdated = ps.executeUpdate();
            System.out.println(rowsUpdated + " contracts(s) marked as CLOSED.");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
